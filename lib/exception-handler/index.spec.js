...
describe('Exception handler', function() {
  ...
  it('should use only the matched error handler', function(done) {
    let invalidHandler = { type: CustomErrorAlt, execute: function() {}};
    let validHandler = { type: CustomError, execute: function() { done(); }};
  ...
  it('should run only the first handler if multiple handlers are matches to the exception', function() {
    let validHandler = { type: CustomError, execute: function() {}};
    let invalidHandler = {
      type: CustomError,
      execute: function() {
  ...
  describe('error code provided', function() {
    ...
    it('should handle error only if error code matches', function(done) {
      let invalidHandler = { code: 401, execute: function() {} };
      let validHandler = {
        code: 400,
        execute: function(err) {
          expect(err).to.be.eql(error);
    ...
    it('should handle error by type and code if both provided', function(done) {
      let invalidHandler1 = { type: CustomError, code: 401, execute: function() {} };
      let invalidHandler2 = { type: CustomErrorAlt, code: 400, execute: function() {} };
      let validHandler = { type: CustomError, code: 400, execute: function() { done(); } };
  ...
  describe('reply code provided', function() {
    ...
    it('should handle error only if reply code matches', function(done) {
      let invalidHandler = { replyCode: 401, execute: function() {} };
      let validHandler = {
        replyCode: 400,
        execute: function(err) {
          expect(err).to.be.eql(error);
  ...
  describe('returnHandler parameter provided', function() {
    ...
    it('should returns the matching handler instead of running it', function(done) {
      let returnedFunction = function() { done(); };

      let matchedFunction = handleException(error, true, {
        type: CustomError,
...
function createError(errorName) {
  let NewError = function(message, code) {
